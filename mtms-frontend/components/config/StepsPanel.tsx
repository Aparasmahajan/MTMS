...
function ChecklistEditor({ list }: { list: StepListView }) {
  ...
  function move(index: number, by: number) {
    ...
    void apply(null, () =>
      send<Snapshot>(`/api/v1/steps/lists/${list.id}/order`, 'PATCH', { entryIds: ids }),
    );
  }
    ...
    <div className="bordered" style={{ marginBottom: 'var(--space-4)' }}>
            ...
            onChange={(event) =>
              void apply(null, () =>
                send<Snapshot>(`/api/v1/steps/lists/${list.id}`, 'PATCH', {
                  enforceOrder: event.target.checked,
                }),
              )
            }
          />
      ...
      <form
        onSubmit={(event) => {
          ...
          if (!adding) return;
          void apply(null, () =>
            send<Snapshot>(`/api/v1/steps/lists/${list.id}/entries`, 'POST', { stepId: adding }),
          ).then((result) => {
            if (result) setAdding('');
          });
        }}
