...
export function initDemoRuntime(snapshot: Snapshot): void {
  // Deep copy so the baked, server-rendered snapshot is never mutated in place — a
  // client-side navigation back to a prerendered page must still show the seed.
  if (!current) current = structuredCloneish(snapshot);
}
...
/** Puts the demo back to the seeded state. */
