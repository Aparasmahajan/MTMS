-- MTMS — the relational target.
--
-- The app currently stores one JSON document (see postgres-driver.ts), which bought
-- multi-process safety without rewriting every mutation. This file is where that goes
-- next: real tables, so the data is queryable by reporting and by anything that is not
-- this Node process.
--
-- NOT YET APPLIED TO A DATABASE. There is no Postgres on the development machine, so this
-- is unverified SQL. See pending.md.
--
...
--
-- Tenancy: every table that holds project data carries tenant_id, and every query filters
-- on it. Row-level security is the eventual belt to this braces — see the note at the end.
...
-- ---------------------------------------------------------------------------
-- Still to decide before this is applied in anger
-- ---------------------------------------------------------------------------
--
-- 1. ROW-LEVEL SECURITY. Tenancy is enforced in the service layer today, and every query
...
--    on a single-column primary key will not map this table.
--
-- 3. MIGRATIONS. This file creates a database from nothing. The document store reseeds on
--    a version bump, which is acceptable for a pilot and is not once real data exists.
--    A migration tool has to be chosen before the first real deployment.
