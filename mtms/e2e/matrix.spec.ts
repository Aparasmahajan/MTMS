...
const ADMIN = { email: 'parmahaj@mail.com', password: 'tracker' };
const VIEWER = { email: 'k.menon@mail.com', password: 'tracker' };
...
test.describe('signing in', () => {
  ...
  test('says the same thing for a wrong password as for an unknown account', async ({ page }) => {
    ...
    await page.getByLabel('Work email').fill('nobody@mail.com');
    await page.getByLabel('Password').fill('wrong');
    await page.getByRole('button', { name: 'Sign in' }).click();
