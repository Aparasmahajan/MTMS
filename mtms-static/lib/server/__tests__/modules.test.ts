...
/**
 * Part 3.3, 3.4 and 3.5 — creating a module, editing its sub_activities, and assigning
 * a defect.
 *
...
async function view(id: string) {
  const found = (await snapshot()).sub_modules.find((module) => module.id === id);
...
describe('creating a module directly', () => {
  it('creates it with every cell blank', async () => {
    ...
    expect(module.module_name).toBe('SBC');
    expect(module.readiness).toBe(0);
    expect(module.sub_activities).toHaveLength(0);
    ...
    expect((await snapshot()).sub_modules).toHaveLength(19);
  });
  ...
  it('allows the same activity name under a different node type', async () => {
    ...
    const matching = (await snapshot()).sub_modules.filter(
      (module) => module.name === 'SHARED_ACTIVITY_NAME',
    );
    expect(matching.map((module) => module.module_name).sort()).toEqual(['CFX', 'SBC']);
  });
  ...
  it('catalogues it when asked', async () => {
    ...
    expect(entry?.module_name).toBe('SBC');
    expect(entry?.used_in_projects).toBe(1);
    expect(entry?.in_this_project).toBe(true);

    const store = await getStore();
    const module = store.sub_modules.find((candidate) => candidate.id === created);
...
describe('adding a subactivity', () => {
  it('carries the module’s own row onto the first one instead of stranding it', async () => {
    ...
    expect(after.sub_activities).toHaveLength(1);
    expect(after.sub_activities[0]!.readiness).toBe(100);

    // The cells moved rather than being copied: nothing is left on the module's own row.
    const cells = await cellsOf(module);
    expect(cells.filter((cell) => cell.sub_activity_id === null)).toHaveLength(0);
    expect(cells.filter((cell) => cell.sub_activity_id === subactivityId)).toHaveLength(26);
  });
  ...
  it('starts a second subactivity blank, which pulls the roll-up back to blank', async () => {
    ...
    const after = await view(module);
    expect(after.sub_activities).toHaveLength(2);
    expect(after.sub_activities[1]!.readiness).toBe(0);
...
describe('renaming a subactivity', () => {
  it('renames it', async () => {
    const module = await moduleId(MODULE_FULL_WITH_SUBS);
    const target = (await view(module)).sub_activities[0]!;

    await renameSubactivity(admin, project, module, target.id, 'Load the announcement set');

    expect((await view(module)).sub_activities[0]!.name).toBe('Load the announcement set');
  });
  ...
  it('is allowed on a closed module, because it changes no status', async () => {
    ...
    expect((await view(module)).sub_activities[0]!.name).toBe('Renamed after closing');
  });
...
describe('removing a subactivity', () => {
  it('takes its cells with it and leaves the others alone', async () => {
    const module = await moduleId(MODULE_FULL_WITH_SUBS);
    const before = await view(module);
    expect(before.sub_activities).toHaveLength(2);
    const first = before.sub_activities[0]!;
    const second = before.sub_activities[1]!;

    await removeSubactivity(admin, project, module, first.id);

    const after = await view(module);
    expect(after.sub_activities).toHaveLength(1);
    expect(after.sub_activities[0]!.id).toBe(second.id);
    expect(after.cells.every((cell) => cell.rolled_up)).toBe(true);

    const cells = await cellsOf(module);
    expect(cells.some((cell) => cell.sub_activity_id === first.id)).toBe(false);
    // The survivor is renumbered so the order stays dense.
    const store = await getStore();
    expect(store.sub_activities.find((sub) => sub.id === second.id)?.order_index).toBe(0);
  });
  ...
  it('hands the module its row back when the last one goes, keeping what it showed', async () => {
    ...
    for (const subactivity of before.sub_activities) {
      await removeSubactivity(admin, project, module, subactivity.id);
    }

    const after = await view(module);
    expect(after.sub_activities).toHaveLength(0);
    expect(after.cells.every((cell) => !cell.rolled_up)).toBe(true);
  ...
  it('materialises the roll-up, not the removed subactivity’s own row', async () => {
    // Two sub_activities disagreeing: one done, one not. The roll-up is the not-done, and
    // that is what the module's row must inherit when the disagreement is removed.
    const module = await moduleId(MODULE_FULL_WITH_SUBS);
    const subs = (await view(module)).sub_activities;
    await advanceCell(admin, project, {
      moduleId: module,
  ...
  it('refuses a closed module', async () => {
    ...
    const target = (await view(module)).sub_activities[0]!;
    await mutate((store) => {
      store.sub_modules.find((candidate) => candidate.id === module)!.fni_closed_at = new Date().toISOString();
    });
